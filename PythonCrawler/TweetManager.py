...
def get_tweets_for_input(tweet_criteria, query_metadata, receive_buffer=None):
    ...
    while active:
        tweet_response_json = Tc.getJsonReponse(tweet_criteria, refresh_cursor)
        ...
        print("Fetched {0} results from Twitter API. {1} / {2}".format(len(temp), len(results),
                                                                       tweet_criteria["max_tweets"]))
        ...
        if len(results) >= tweet_criteria["max_tweets"] >= 0:
            active = False
